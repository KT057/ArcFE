...
export const renderPugs = async (
  ...
  const promises = resolve.map((dir) => {
    ...
    return render(dir, outputPath, outPutFile);
  });
  ...
  if (errs.length) {
    log('error', `Rendering errors occurred`);
...
// 変更を監視
export const watchPug = async (entry: string, option: GlobOptions = {}) => {
  const { resolve, reject } = await getDirsSync(entry, option);
  ...
  chokidar.watcher({
    change: async (path: string) => {
      ...
      await renderPugs(
        ...
        {
          ignore: [
            `${path.split(DIR.TEMPLATE)[0]}/${DIR.TEMPLATE}/**/_*${EXTENSION.PUG}`
          ]
        }
      );
    }
